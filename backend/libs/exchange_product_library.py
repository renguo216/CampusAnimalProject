...
class ExchangeProductLibrary:
    ...
    def get_exchange_items(self, page=1, page_size=20):
        ...
        try:
            ...
            # 直接查询所有商品，不限制 status 先看看有没数据
            all_result = self.db.get_paginated(
                't_exchange_product',
                page=1,
                page_size=100,
                where_clause="1=1",
            ...
            for item in data_list:
                ...
                items.append({
                    ...
                    'image': image_url,
                    'points': int(item.get('points_required', 0)),
                    'stock': int(item.get('stock', 0))
                })
            
            print(f"返回商品数据: {items}")
            return success_response("成功", data=items)
