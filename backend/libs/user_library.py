...
class UserLibrary:
    ...
    def update_user_info(self, user_id, update_data) -> dict:
        ...
        # 先检查用户是否存在，不检查is_active，避免"已注销"问题
        user = self._get_user_by_account(user_id)
        if not user:
    ...
    def get_all_users(self, page=1, page_size=20) -> dict:
        """
        获取所有用户列表（分页）
        :return: dict {"success": bool, "message": str, "data": {"total": int, "page": int, "page_size": int, "users": list}}
        """
        if not self.db.open_database():
            return error_response("数据库连接失败")
        ...
        sql_data = """
            SELECT user_id, nickname, avatarURL, role, points, phone, created_at 
            FROM t_user 
            ORDER BY created_at DESC 
            LIMIT %s OFFSET %s
        """
        results = self.db.execute_raw_sql(sql_data, (page_size, offset))
        
        sql_count = "SELECT COUNT(*) AS total FROM t_user"
